...
<html lang="en">
   <head>
      ...
      <title>Noticias Portal  | Pagina de Inicio</title>
               ...
               <div class="card border-0">
                  <div class="card-body">
                    <a class="badge bg-success text-decoration-none link-light" href="category.php?catid=<?php echo htmlentities($row['cid'])?>" style="color:#fff"><?php echo htmlentities($row['category']);?></a>
                     ...
                     <p>
                         Feha <?php echo htmlentities($row['postedBy']);?> de Creacion| <?php echo htmlentities($row['postingdate']);?>
                        <?php if($row['lastUpdatedBy']!=''):?>
                        siguiente actualizacion <?php echo htmlentities($row['lastUpdatedBy']);?> on<?php echo htmlentities($row['UpdationDate']);?>
                     </p>
                     <?php endif;?>
                     <p><strong>Compartir:</strong> <a href="http://www.facebook.com/share.php?u=<?php echo $currenturl;?>" target="_blank">Facebook</a> | 
          ...
          <div class="col-md-8">
            ...
            <div class="col-md-8">
               ...
               <div class="card my-4 bg-transparent border-0">
                  <h5 class="card-header bg-transparent border-0">Comentar</h5>
                  <div class="card-body">
                     <form name="Comment" method="post">
                        <input type="hidden" name="csrftoken" value="<?php echo htmlentities($_SESSION['token']); ?>" />
                        <div class="form-group">
                           <input type="text" name="name" class="form-control rounded-0" placeholder="Nombre Completo" required>
                        </div>
                        <div class="form-group">
                           <input type="email" name="email" class="form-control rounded-0" placeholder="email" required>
                        </div>
                        <div class="form-group">
                           <textarea class="form-control rounded-0" name="comment" rows="3" placeholder="Commentario" required></textarea>
                        </div>
                        <div class="mb-3 form-group">
                         <div class="g-recaptcha" data-sitekey="6LcJPRYpAAAAAOjVvJTkXABKRpJOkjhFXdETmSTL"></div>
                        </div>
                        <button type="submit" class="btn btn-danger" name="submit">Comentar</button>
                     </form>
                  </div>
               </div>
         </div>
